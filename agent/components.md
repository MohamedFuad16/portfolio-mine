...
- `ContributionGrid`: GitHub-style activity grid populated from the snapshot, then the public MohamedFuad16 contributions API. A stats row (total, active days, longest/current streak, busiest day) is computed from the drawn cells by `summariseContributions`. One tooltip is portaled into `<body>` at the hovered cell and re-anchored on scroll; month labels are buttons that dim other months and put that month's total in the caption; the grid takes arrow keys (up/down = day, left/right = week, Home/End). Phones scroll only `.calendar-viewport`, opened at the recent end (ADR-061).
